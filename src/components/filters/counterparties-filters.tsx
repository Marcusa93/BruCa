...
export function CounterpartiesFilters({
  ...
  active: { q: string; risk: string; status: string };
}) {
  const router = useRouter();
  const params = useSearchParams();

  function update(patch: Partial<{ q: string; risk: string; status: string }>) {
  ...
  const hasFilters =
    active.q !== "" || active.risk !== "all" || active.status !== "all";
    ...
    <Card>
      <CardContent className="flex flex-wrap items-center gap-3 py-4">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input
            placeholder="Buscar nombre, alias, documento, banco…"
            value={active.q}
        ...
        <label className="flex items-center gap-2 text-xs">
          <span className="text-ink-3">Riesgo</span>
          <select
            value={active.risk}
