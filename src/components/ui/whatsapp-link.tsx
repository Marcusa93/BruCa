...
/** Logo oficial de WhatsApp (verde, hereda currentColor). */
...
export function PhoneWithWhatsApp({
  ...
  if (!phone) return <span className={cn("text-ink-3", className)}>—</span>;
    ...
    <span className={cn("inline-flex items-center gap-2", className)}>
      ...
      {wa && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[#25D366] text-white shadow-sm transition-transform hover:scale-110"
          title={`Chatear por WhatsApp con ${phone}`}
